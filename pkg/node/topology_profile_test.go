...
func makeKnownPeers(public, sameSitePrivate, remotePrivate int) ([]string, []store.KnownPeer) {
	localIPs := []string{"10.1.0.1"}
	peers := make([]store.KnownPeer, 0, public+sameSitePrivate+remotePrivate)
	id := byte(1)
...
func TestAdaptiveParamsTinyCluster(t *testing.T) {
	...
	require.Equal(t, topology.DefaultNearestK, p.NearestK)
	require.Equal(t, topology.DefaultRandomR, p.RandomR)
	require.Equal(t, topology.DefaultInfraMax, p.InfraMax)
}
