...
// WorkloadInvoker executes a function on a locally compiled workload.
...
// The context controls the lifetime — cancelling it closes the stream to abort the remote call.
func InvokeOverStream(ctx context.Context, stream io.ReadWriteCloser, hash, function string, input []byte) ([]byte, error) {
	defer stream.Close()

	// Close the stream if the context is cancelled, aborting any in-flight I/O.
	// Use a done channel to ensure the goroutine exits when the call completes.
	...
	case statusOK:
		return body, nil
	case statusNotFound:
		return nil, fmt.Errorf("invoke: workload not found: %s: %w", string(body), workload.ErrNotRunning)
	case statusError:
...
// ctxOrWrap returns ctx.Err() if the context is done (so callers see
// context.DeadlineExceeded or context.Canceled), otherwise wraps the
// original I/O error.
