...
func TestEvaluate_UnderReplicated_Claims(t *testing.T) {
	...
	actions := evaluate(local, allPeers, specs, claims, nil, isRunning)
...
func TestEvaluate_OverReplicated_LoserReleases(t *testing.T) {
	...
	actions2 := evaluate(peer2, allPeers, specs, claims, nil, isRunning)
...
func TestEvaluate_IndependentNodesAgreeOnWinner(t *testing.T) {
	...
	for _, pk := range peers {
		actions := evaluate(pk, peers, specs, claims, nil, isRunning)
		for _, a := range actions {
...
func TestSuitabilityScore_HashFallback(t *testing.T) {
	...
	specs := map[string]uint32{"workload": 1}
	claims := map[string]map[types.PeerKey]struct{}{}
	isRunning := func(string) bool { return false }
...
func TestEvaluate_IndependentNodesAgreeWithClusterState(t *testing.T) {
	peers := make([]types.PeerKey, 5)
	nodes := make(map[types.PeerKey]store.NodePlacementState, 5)
	for i := range peers {
		peers[i] = peerKey(byte(i + 1))
		nodes[peers[i]] = store.NodePlacementState{
	...
	specs := map[string]uint32{"workload1": 2}
	claims := map[string]map[types.PeerKey]struct{}{}
	isRunning := func(string) bool { return false }
...
func TestEvaluate_MigrationOnTrafficShift(t *testing.T) {
	...
	cluster1 := map[types.PeerKey]store.NodePlacementState{
		p1: {NumCPU: 4, CPUPercent: 50, MemTotalBytes: 8 << 30, MemPercent: 50},
		p2: {NumCPU: 4, CPUPercent: 50, MemTotalBytes: 8 << 30, MemPercent: 50},
	...
	actions := evaluate(p1, allPeers, map[string]uint32{hash: 1}, claims1, cluster1, isRunning)
	...
	cluster2 := map[types.PeerKey]store.NodePlacementState{
		p1: {NumCPU: 4, CPUPercent: 50, MemTotalBytes: 8 << 30, MemPercent: 50},
		p2: {
			NumCPU: 4, CPUPercent: 50, MemTotalBytes: 8 << 30, MemPercent: 50,
	...
	actions = evaluate(p1, allPeers, map[string]uint32{hash: 1}, claims1, cluster2, isRunning)
	p1Releasing = false
	for _, a := range actions {
		if a.Hash == hash && a.Kind == actionRelease {
			p1Releasing = true
	...
	actions = evaluate(p2, allPeers, map[string]uint32{hash: 1}, claimsAfterRelease, cluster2, func(string) bool { return false })
	var p2Claiming bool
	for _, a := range actions {
...
func TestEvaluate_MigrationInhibitedByIncumbentBonus(t *testing.T) {
	...
	actions := evaluate(p1, allPeers, map[string]uint32{hash: 1}, claims, cluster, func(string) bool { return true })
	var releasing bool
	for _, a := range actions {
		if a.Hash == hash && a.Kind == actionRelease {
			releasing = true
...
func TestEvaluate_OverReplicatedWithBetterNonClaimant(t *testing.T) {
	...
	for _, pk := range allPeers {
		actions := evaluate(pk, allPeers, map[string]uint32{hash: 1}, claims, cluster, func(string) bool { return true })
		for _, a := range actions {
			if a.Hash == hash && a.Kind == actionRelease {
				releasers++
			}
...
func TestEvaluate_DeadNodeExcludedFromCandidates(t *testing.T) {
	...
	actions := evaluate(local, livePeers, specs, claims, nil, isRunning)
	...
	actionsWithDead := evaluate(local, allPeersIncludingDead, specs, claims, nil, isRunning)
